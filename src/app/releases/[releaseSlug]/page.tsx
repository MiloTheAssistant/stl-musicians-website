...
export default async function ReleaseSmartLinkPage({
  ...
  return (
    <>
      ...
      />
      <main>
        <section className="border-b border-[var(--line)] bg-[var(--muted)]">
          <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(320px,0.7fr)] lg:px-8">
            <div>
              <Eyebrow>SmartLink release page</Eyebrow>
              <h1 className="mt-4 max-w-3xl text-4xl font-black leading-tight text-[var(--ink)] sm:text-6xl sm:leading-[0.98]">
                {release.title}
              </h1>
              <p className="mt-4 text-xl font-bold text-[var(--ink)]">
                {release.bandName} · {release.releaseType.toUpperCase()} ·{" "}
                {releaseDate}
              </p>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-[rgba(38,31,24,0.82)]">
                {release.summary}
              </p>
            ...
            {release.artworkUrl ? (
              <div className="mx-auto w-full max-w-[440px]">
                <Image
                  ...
                  height={900}
                  priority
                  className="aspect-square w-full rounded-md border border-[rgba(38,31,24,0.18)] object-cover shadow-2xl"
                />
              </div>
            ) : null}
          </div>
        ...
        <SectionShell className="grid gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(320px,0.55fr)]">
          <div>
            ...
            <h2 className="mt-3 text-3xl font-black">Release destinations</h2>
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {release.destinationLinks.map((link) => (
                <a
                  key={link.id}
                  href={`${releasePath}/go/${link.id}?source=destination-card`}
                  className="rounded-md border border-[var(--line)] bg-[rgba(245,234,210,0.06)] p-5 transition hover:border-[var(--brass)] hover:bg-[rgba(245,234,210,0.1)]"
                >
